...
UTILS.getSectorArmy = function(sector, tile, isCountBuildings)
	local army
	army = UTILS.sortObjectsByFactions(UTILS.getSectorObjects(tile, sector), isCountBuildings)
	return UTILS.convertArmyToArray(army)
end

function UTILS.convertArmyToArray(army)
	local result = {}
...
function UTILS.sortObjectsByFactions(objs, isCountBuildings)
	local army = {}
...
function UTILS.getSectorObjects(tile, sector)
	local position = tile.getPosition()
	local signX = { -1, 1, 1, -1 }
	local signZ = { 1, 1, -1, -1 }
	local data, objs = {}, {}
...
UTILS.getFactionDataByColor = function(color)
