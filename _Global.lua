...
function fightClicked()
	local status, err =
		BATTLE_SCRIPTS.checkEligibleBattles(STORE.boardZone, COMBAT.botFightZoneGUID, COMBAT.topFightZoneGUID)
	if status then
...
function updateMateriels()
	...
	for faction, data in pairs(STORE.factionsData) do
		...
		for _, player in ipairs(Player.getPlayers()) do
			if player.color == data.color then
				isSeatedPlayer = true
			end
		end
		...
		if isSeatedPlayer then
			...
			local materiel = data.name
				.. ": "
				.. STORE.factionsNameFiller[faction]
				.. "materiel "
				.. old
				.. " + "
				.. delta
	...
	for _, occupyingObject in ipairs(zone.getObjects()) do
		...
		if occupyingObject.type == "Dice" or string.find(name, "token") then
			...
			if occupyingObject.type == "Figurine" then
				if
					string.find(occupyingObject.getName(), "City")
					or string.find(occupyingObject.getName(), "Factory")
					or string.find(occupyingObject.getName(), "Bastion")
				then
					occupyingObject.destruct()
				else
...
function toggleBorder(tile)
	...
	if STORE.showSectorBorders then
		...
		for i = 1, 4 do
			...
			if #army == 1 then
				lines =
					addSectorOccupationLines(adjustSector(i, tile), army[1].faction, lines, index, tile.is_face_down)
				index = index + 4
			end
		end
	end
...
function addSectorOccupationLines(sector, faction, lines, index, tileFlipped)
	...
	lines[index] = createLine(
		{ { xz - l - xOffA, y, xz - yOffA }, { xz - xOffA, y, xz - yOffA }, { xz - xOffA, y, xz - l - yOffA } },
		c
	)
	lines[index + 1] = createLine(
		{ { g + l - xOffA, y, xz - yOffA }, { g - xOffA, y, xz - yOffA }, { g - xOffA, y, xz - l - yOffA } },
		c
	)
	lines[index + 2] = createLine(
		{ { xz - l - xOffA, y, g - yOffA }, { xz - xOffA, y, g - yOffA }, { xz - xOffA, y, g + l - yOffA } },
		c
	)
	lines[index + 3] =
		createLine({ { g + l - xOffA, y, g - yOffA }, { g - xOffA, y, g - yOffA }, { g - xOffA, y, g + l - yOffA } }, c)
	return lines
end
...
function onObjectLeaveScriptingZone(zone, obj)
	...
	-- if obj.isDestroyed() then return end -- this might interfere actually
...
function calculate()
	...
	if not bottomFightZoneObjects then
		return
	end
